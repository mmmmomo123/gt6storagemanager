...
- **扫描**：以管理器为中心的轴对齐盒，范围可配置（默认 8 = 上下左右各 8 格，最大 8），最多管 26 个桶。
- **范围控制 GUI**：空手右键管理器打开，可调 X/Y/Z 偏移（±32）与半径（0~16），可开关自定义范围；关闭时回退默认立方体。
- **范围可视化**：GUI 内开关；开启后管理器附近会在大世界画出绿色“火柴盒”线框（未启用自定义范围时画灰色），`config/gtsm.cfg` 只存全局默认值，实际范围存在每个方块的 NBT 里。
